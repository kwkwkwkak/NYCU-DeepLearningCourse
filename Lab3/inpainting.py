...
#mask_b: iteration decoding initial mask, where mask_b is true means mask
    def inpainting(self,image,mask_b,i): #MakGIT inference
        ...
        with torch.no_grad():
            ...
            z_indices = z_indices.view(1, -1)
            #z_indices[mask_b] = self.model.mask_token_id #z_indices: masked tokens (b,16*16)
            ...
            for step in range(self.total_iter):
                if step == self.sweet_spot:
                    break
                #ratio = self.model.gamma(i / (self.total_iter - 1)) #this should be updated
    
                z_indices_predict, mask_bc = self.model.inpainting(z_indices_predict, mask_bc, mask_num, step, self.total_iter)
            ...
            # for ind, im in enumerate(imga):
            #     vutils.save_image(im, os.path.join("test_results", f"image_{i:03d}_ind{ind}.png"), nrow=1) 
...
#TODO3 step1-2: modify the path, MVTM parameters
    parser.add_argument('--load-transformer-ckpt-path', type=str, default='checkpoints/best_ckpt.pt', help='load ckpt')
