...
class Actor(nn.Module):
    ...
    def forward(self, state: torch.Tensor) -> torch.Tensor:
        ...
        std = self.logstds.clamp(self.logstds_min, self.logstds_max).exp()
...
class PPOAgent:
            ...
            for _ in range(self.rollout_len):
                ...
                if done[0][0]:
                    ...
                    pbar.set_description(
                        f"Episode {episode_count}: Total Reward = {score}"
                    )
                    if self.logging:
                        wandb.log({
    ...
    def save_models(self, save_dir: str = "ppo_pendulum_models", ep: int = None):
    ...
    def load_models(self, save_dir: str = "ppo_pendulum_models", ep: int = None):
        actor_path = os.path.join(
            save_dir, f"actor{'_' + str(ep) if ep is not None else ''}.pt"
        )
        critic_path = os.path.join(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--wandb-run-name", type=str, default="pendulum-ppo-run-final")
    ...
    #agent.load_models(save_dir="ppo_pendulum_models_final", ep=106001)
    
    agent.load_models_(save_dir="../Lab7_110550022_task2_ppo_pendulum/")
    # the loaded model is from step 106001
    seeds = [1, 2, 6, 7, 8, 12, 15, 16, 18, 19, 20, 22, 23, 24, 25, 26, 27, 30, 33, 35]
    for seed in seeds:
        agent.seed = seed
        agent.test()
    #agent.test("output_pendulum")
